class AggregatorTrendlines < AggregatorBase
  ...
  def add_trendlines_for_rails_all_points
    results.series_manager.trendlines.each do |trendline_series_name|
      model_type_for_trendline = Series::ManagerBase.series_name_for_trendline(trendline_series_name)
      trendline_name_with_parameters = add_regression_parameters_to_trendline_symbol(trendline_series_name, model_type_for_trendline)
      results.bucketed_data[trendline_name_with_parameters] = Array.new(@x_axis.length, Float::NAN)
  ...
  def add_trendlines_for_rails_2_points
    ...
    series_model_types.each do |model_type|
      model_temperatures_and_index = results.bucketed_data[model_type].each_with_index.map { | kwh, index| kwh.nan? ? nil : [temperatures[index], index, results.x_axis[index]] }.compact
      ...
      bucketed_data[trendline] = Array.new(results.x_axis.length, Float::NAN)
      bucketed_data[trendline][min[1]] = results.series_manager.predicted_amr_data_one_day(min[2]) * trendline_scale
      bucketed_data[trendline][max[1]] = results.series_manager.predicted_amr_data_one_day(max[2]) * trendline_scale
    end
  end
