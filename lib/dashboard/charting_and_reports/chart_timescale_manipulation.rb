...
class ChartManager
  ...
  def adjust_timescale(chart_name, chart_config_original, command, factor)
    logger.info "Adjust timescale request for chart: #{chart_name} command: #{command} factor: #{factor}"
    case command
    when :move
      move_timescale(chart_name, chart_config_original, factor)
    when :extend
      extend_timescale(chart_name, chart_config_original, factor)
    when :contract
      contract_timescale(chart_name, chart_config_original, factor)
    when :compare
      compare_timescale(chart_name, chart_config_original, factor)
    else
      throw EnergySparksUnexpectedStateException.new('Unexpected nil chart adjustment timescale shift') if command.nil?
      throw EnergySparksUnexpectedStateException.new("Unexpected chart adjustment timescale shift #{command}")
    end
  end
  ...
  def move_timescale(old_chart_name, chart_config_original, factor)
    logger.info "Move timescale request"
    available_periods = available_periods(chart_config_original)
    logger.info "#{available_periods} periods available for chart time manipulation"
    new_config = chart_config_original.deep_dup
    timescales = convert_timescale_to_array(new_config)

    new_timescales = move_timescales(timescales, factor, available_periods)
    ...
    logger.info "Move timescale request complete"
    new_config
  end

  def move_timescales(timescales, factor, available_periods)
    new_timescales = []
    timescales.each do |timescale|
      new_timescales.push(manipulate_timescale_move(timescale, factor, available_periods))
    end
    new_timescales
  end

  def manipulate_timescale_move(timescale, factor, available_periods)
  ...
  def extend_timescale(old_chart_name, chart_config_original, factor)
    logger.info "Extend timescale request"
    ...
    new_timescales = extend_timescales(timescales, factor, available_periods)
    ...
    logger.info "Extend timescale request complete"
    new_config
  end

  def extend_timescales(timescales, factor, available_periods)
    new_timescales = []
    timescales.each do |timescale|
      new_timescales.push(manipulate_timescale_extend(timescale, factor, available_periods))
    end
    new_timescales
  end

  def manipulate_timescale_extend(timescale, factor, available_periods)
    period_type, period_number = timescale.first
    if period_number.is_a?(Integer)
      new_period_number = calculate_new_period_number(period_number, factor, available_periods)
  ...
  def contract_timescale(old_chart_name, chart_config_original, factor)
    logger.info "Extend timescale request"
    ...
    new_timescales = contract_timescales(timescales, factor, available_periods)
  ...
  def contract_timescales(timescales, factor, available_periods)
    new_timescales = []
    timescales.each do |timescale|
      new_timescales.push(manipulate_timescale_contract(timescale, factor, available_periods))
    end
    new_timescales
  end

  def manipulate_timescale_contract(timescale, factor, available_periods)
    period_type, period_number = timescale.first
    if period_number.is_a?(Integer)
  ...
  def compare_timescale(old_chart_name, chart_config_original, factor)
    logger.info "Compare timescale request"
    ...
    new_timescales = compare_timescales(timescales, factor, available_periods)
    ...
    logger.info "Compare timescale request complete"
    new_config
  end

  def compare_timescales(timescales, factor, available_periods)
    new_timescales = []
  ...
  def manipulate_timescale_compare(timescale, factor, available_periods)
    period_type, period_number = timescale.first
    if period_number.is_a?(Integer)
      new_period_number = calculate_new_period_number(period_number, factor, available_periods)
      {period_type => new_period_number}
