...
require_relative './benchmark_content_general'
require_relative '../charting_and_reports/tables/management_summary_table'
  ...
  class BenchmarkManager
    ...
    def self.benchmark_titles_for(benchmark_keys, user_role)
      benchmark_keys.each_with_object({}) do |benchmark_key, benchmarks|
        next if CHART_TABLE_CONFIG[benchmark_key][:admin_only] == true && %i[admin analyst].exclude?(user_role)
    ...
    def self.available_pages(filter_out = nil)
      ...
      unless filter_out.nil?
        all_pages = all_pages.reject { |_name, config| filter_out?(config, filter_out.keys[0], filter_out.values[0]) }
      end
    ...
    CHART_TABLE_GROUPING = {
      ...
      gas_and_storage_heater_benchmarks: %i[
    ...
    TARIFF_CHANGED_COL = { data: -> { addp_etch || addp_gtch }, name: :tariff_changed, units: TrueClass, hidden: true }.freeze
    ...
    CHART_TABLE_CONFIG = {
      ...
      change_in_energy_since_last_year: {
        ...
        columns: [
          ...
          { data: lambda {
                    sum_if_complete([enba_ken, enba_kgn, enba_khn, enba_ksn],
                                    [enba_ke0, enba_kg0, enba_kh0, enba_ks0])
                  }, name: :previous_year, units: :kwh },
          ...
          { data: lambda {
                    sum_if_complete([enba_cen, enba_cgn, enba_chn, enba_csn],
                                    [enba_ce0, enba_cg0, enba_ch0, enba_cs0])
                  }, name: :previous_year, units: :co2 },
          ...
          { data: lambda {
                    sum_if_complete([enba_pen, enba_pgn, enba_phn, enba_psn],
                                    [enba_pe0, enba_pg0, enba_ph0, enba_ps0])
                  }, name: :previous_year, units: :£ },
          ...
          {
            data: lambda {
                    if enba_peap == ManagementSummaryTable::NO_RECENT_DATA_MESSAGE ||
                       enba_pgap == ManagementSummaryTable::NO_RECENT_DATA_MESSAGE
                      'Y'
                    else
                      ''
                    end
