  ...
  class SunAngleOrientation
    ...
    def e2
      (@datetime.hour / 24.0) + (@datetime.minute / 60.0 / 24.0)
    end
    ...
    def v2
      4.0*degrees(u2*sin(2.0*radians(i2))-2.0*k2*sin(radians(j2))+4.0*k2*u2*sin(radians(j2))*cos(2.0*radians(i2))-0.5*u2*u2*sin(4.0*radians(i2))-1.25*k2*k2*sin(2.0*radians(j2)))
    end
    alias_method :eq_of_time, :v2

    def ab2
      mod(e2*1440+v2+4*b4-60*b5,1440)
    end
    alias_method :true_solar_time, :ab2

    def ac2
      iff(ab2/4<0,ab2/4+180,ab2/4-180)
    end
    alias_method :hour_angle_deg, :ac2

    def d2
      (date - Date.new(1899,12,30)).to_f
    end
    alias_method :excel_date, :d2

    def f2
      d2+2415018.5+e2-b5/24.0
    end
    ...
    def ad2
      degrees(acos(sin(radians(b3))*sin(radians(t2))+cos(radians(b3))*cos(radians(t2))*cos(radians(ac2))))
    end
    ...
    def af2
      iff(ae2>85,0,iff(ae2>5,58.1/tan(radians(ae2))-0.07/power(tan(radians(ae2)),3)+0.000086/power(tan(radians(ae2)),5),iff(ae2>-0.575,1735+ae2*(-518.2+ae2*(103.4+ae2*(-12.79+ae2*0.711))),-20.772/tan(radians(ae2)))))/3600
    end
    ...
    def ah2
      iff(ac2>0,mod(degrees(acos(((sin(radians(b3))*cos(radians(ad2)))-sin(radians(t2)))/(cos(radians(b3))*sin(radians(ad2)))))+180,360),mod(540-degrees(acos(((sin(radians(b3))*cos(radians(ad2)))-sin(radians(t2)))/(cos(radians(b3))*sin(radians(ad2))))),360))
    end
