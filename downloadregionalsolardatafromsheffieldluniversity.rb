...
@climate_zones = { # probably not the same climate zones as the other inputs, more critical they are local, so schools to west of Bath may need their own 'climate zone'
  ...
  'Crewkerne' => {
    ...
    longitude: -2.772,
    proxies: [
      { id: 10, name: 'Axminster', code: 'AXMI', latitude: 50.78594, longitude: -2.92337 },
      { id: 40, name: 'Bridgewater', code: 'BRWA', latitude: 51.1168, longitude: -2.9677 },
      { id: 58, name: 'Chickerell', code: 'CHIC', latitude: 50.62406, longitude: -2.48521 }
    ]
  },
  'Portworthy' => {
    latitude: 50.43245,
    longitude: -4.02246,
    proxies: [
      { id: 10, name: 'Landulph', code: 'LAND', latitude: 50.44447, longitude: -4.24153 },
      { id: 1, name: 'Adham', code: 'ABHA', latitude: 50.47108, longitude: -3.72977 }
    ]
  }
}
...
def remove_outliers(data, distances, names, datetime)
  ...
  (0..data.length - 1).each do |i|
    if data[i] > median_yield + @yield_diff_criteria || data[i] < median_yield - @yield_diff_criteria
      bad_indexes.push(i)
      message = "Warning: rejecting yield for #{names[i]} value #{data[i]} from values #{data} for #{datetime}"
      puts message
      @errors.push(message)
    end
  end
...
def remove_nil_values(data, names, distances, datetime, latitude, longitude)
  ...
  # then remove them from the analysis
  bad_indexes.each do |ii|
    data.delete_at(ii)
    names.delete_at(ii)
    distances.delete_at(ii)
  end
end

def calculate_average(data, names, distances, latitude, longitude, datetime, _criteria)
...
def process_regional_data(regional_data, start_date, end_date, latitude, longitude)
  ...
  start_time.step(end_time, thirty_minutes_step).each do |dt_30mins|
    ...
    regional_data.each_value do |region_data|
