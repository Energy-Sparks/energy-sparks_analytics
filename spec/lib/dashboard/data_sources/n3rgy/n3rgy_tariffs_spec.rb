  ...
  describe '#parameterise' do
    ...
    context 'with basic and tiered tariffs' do
      ...
      context 'with kwh tariffs' do
        it 'returns kwh_rates' do
          expect(results[:kwh_rates]).not_to be_nil
        end

        it 'returns expected ranges' do
          expect(results[:kwh_rates][single_day..single_day]).not_to be_nil
          expect(results[:kwh_rates][start_date..end_date]).not_to be_nil
          expect(results[:kwh_rates][single_day_tiered..single_day_tiered]).not_to be_nil
        end
      ...
      context 'with standing charges' do
        it 'returns standing changes' do
          expect(results[:standing_charges]).not_to be_nil
        end

        it 'summarises standing charges' do
          expect(results[:standing_charges][start_date..N3rgyTariffs::INFINITE_DATE]).to be(0.0025)
        end
      end
    end
    ...
    describe 'with Weekday Tariffs' do
      ...
      it 'groups them correctly' do
        ...
        expect(tariffs[until_seven]).to be(0.0641)
        expect(tariffs[from_seven]).to be(0.1402)
        ...
        expect(tariffs[until_seven]).to be(0.0641)
        expect(tariffs[seven_to_four]).to be(0.1402)
        expect(tariffs[four_to_eight]).to be(0.2999)
        expect(tariffs[eight_to_midnight]).to be(0.1402)
