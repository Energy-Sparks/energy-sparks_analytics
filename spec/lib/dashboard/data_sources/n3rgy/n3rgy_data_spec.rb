...
describe MeterReadingsFeeds::N3rgyData do
  describe 'when querying data' do
    ...
    let(:api) { described_class.new(api_key: apikey, base_url: base_url) }
    ...
    describe 'status' do
      ...
      context 'when in DCC and consented' do
        let(:response) do
          { 'entries' => %w[gas electricity], 'resource' => '/2234567891000/',
    ...
    describe 'list' do
      context 'when consented meters exist' do
        ...
        it 'returns array of MPANs' do
          expect(api.list).to match_array(%w[1234567891000 1234567891002 1234567891008])
        end
      end
    end
    ...
    describe 'for tariffs' do
      ...
      describe 'when date not specified' do
        it 'raises error' do
          ...
          end.to raise_error(MeterReadingsFeeds::N3rgyData::BadParameters)
        end
      end
      ...
      describe 'when data exists' do
        ...
        describe 'for normal tariffs' do
          ...
          it 'returns tariffs without duplicates' do
            tariffs = api.tariffs(mpxn, fuel_type, start_date, end_date)
            expect(tariffs.keys).to match_array(%i[kwh_tariffs standing_charges missing_readings])
          ...
          it 'deduplicates standing charges' do
            tariff_data['values'][0]['standingCharges'] =
              [{ 'startDate' => '2019-01-01', 'value' => original_standing_charge }]
            tariff_data['values'][0]['standingCharges'] << { 'startDate' => '2019-01-02', 'value' => original_standing_charge } # should be removed
            tariff_data['values'][0]['standingCharges'] << { 'startDate' => '2019-01-03',
                                                             'value' => original_standing_charge + 1 }
            tariff_data['values'][0]['standingCharges'] << { 'startDate' => '2019-01-04',
                                                             'value' => original_standing_charge }
            ...
            expect(tariffs[:standing_charges].keys.map(&:to_s)).to match_array(%w[2019-01-01 2019-01-03
                                                                                  2019-01-04])
          end

          describe 'when adjusting for bad sandbox electricity standing charge units' do
            it 'converts standing charge from pence to pounds' do
              tariffs = described_class.new(api_key: apikey, base_url: base_url, bad_electricity_standing_charge_units: true).tariffs(
                mpxn, fuel_type, start_date, end_date
              )
              expect(tariffs[:standing_charges][start_date]).to eq(100.0 * expected_standing_charge)
            end

            it 'stills convert prices from pence to pounds' do
              tariffs = described_class.new(api_key: apikey, base_url: base_url, bad_electricity_standing_charge_units: true).tariffs(
                mpxn, fuel_type, start_date, end_date
              )
        ...
        describe 'for tiered tariffs' do
          let(:expected_tiered_tariff) do
            { tariffs: { 1 => 0.48527000000000003, 2 => 0.16774 }, thresholds: { 1 => 1000 }, type: :tiered }
          end
          let(:expected_first_day_tariffs) do
            [expected_tiered_tariff, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992,
             0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992, 0.15992]
          end
          let(:expected_standing_charge)    { 0.19541 }

          let(:tariff_data)                 do
            JSON.parse(File.read('spec/fixtures/n3rgy/get_tariff_data_with_tiers.json'))
          end
    ...
    describe 'for consumption' do
      let(:expected_first_day_readings) do
        [0.671, 1.212, 1.208, 0.972, 0.445, 0.43, 0.35, 0.388, 0.366, 0.449, 0.374, 0.381, 0.412, 0.464, 0.38, 0.317, 0.313,
         0.488, 0.529, 1.96, 0.839, 0.554, 1.062, 1.635, 0.734, 0.561, 0.518, 0.407, 0.362, 0.291, 0.28, 0.349, 0.32, 0.415, 0.355, 0.318, 0.321, 0.347, 0.409, 0.406, 0.354, 0.362, 0.311, 0.439, 0.439, 0.38, 0.39, 0.426]
      end
      let(:expected_last_day_readings) do
        [0.405, 0.479, 0.463, 0.528, 0.517, 0.589, 0.554, 0.599, 0.595, 0.648, 0.574, 0.674, 0.633, 0.713, 0.585, 0.562,
         0.481, 0.516, 0.459, 0.473, 0.399, 0.459, 0.462, 0.496, 0.51, 0.478, 0.369, 0.482, 0.433, 0.416, 0.403, 0.451, 0.406, 0.386, 0.417, 0.4, 0.377, 0.532, 0.637, 0.688, 0.736, 0.643, 0.621, 0.642, 0.791, 1.331, 0.512, 0.674]
      end
      ...
      describe 'when data exists' do
        ...
        it 'returns readings' do
          ...
          expect(readings[fuel_type].keys).to match_array(%i[mpan_mprn readings missing_readings])
    ...
    describe 'elements' do
      ...
      before do
        expect_any_instance_of(MeterReadingsFeeds::N3rgyDataApi).to receive(:get_elements).with(mpxn: mpxn,
                                                                                                fuel_type: :electricity, reading_type: 'consumption').and_return(response)
      end
    ...
    describe 'for inventory' do
      ...
      let(:inventory_data)     do
        { 'status' => status, 'uuid' => '3b80564b-fa21-451a-a8a1-2b4abb6bb8f6', 'uri' => inventory_url }
      end
      let(:inventory_file) do
        { 'result' => [{ 'mpxn' => '1234567891234', 'status' => 404, 'message' => 'MPxN not found' }] }
      end
      ...
      before do
        ...
        expect_any_instance_of(MeterReadingsFeeds::N3rgyDataApi).to receive(:fetch).with(inventory_url, retry_interval,
                                                                                         max_retries).and_return(inventory_file)
      end
