...
describe X48Formatter do
  ...
  describe 'when no readings' do
    ...
    it 'returns hash' do
      response = X48Formatter.convert_dt_to_v_to_date_to_v_x48(start_date, end_date, dt_to_kwh)
      expect(response.keys).to eq([:readings, :missing_readings])
  ...
  describe 'when partial readings' do
    ...
    it 'starts at 0030, ends at 0000 next day' do
      ...
      response = X48Formatter.convert_dt_to_v_to_date_to_v_x48(start_date, end_date, dt_to_kwh, true)
      expect(response.keys).to eq([:readings, :missing_readings])
