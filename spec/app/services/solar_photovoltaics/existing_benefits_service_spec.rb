...
describe SolarPhotovoltaics::ExistingBenefitsService, type: :service do
  ...
  context '#create_model' do
    it 'calculates the existing benefits for a school with solar pv' do
      ...
      expect(benefits.annual_electricity_including_onsite_solar_pv_consumption_kwh).to round_to_two_digits(59417.55)
      expect(benefits.annual_carbon_saving_percent).to round_to_two_digits(0.22)
      expect(benefits.saving_£current).to round_to_two_digits(1689.02)
      ...
      expect(benefits.annual_solar_pv_kwh).to round_to_two_digits(13262.58)
      expect(benefits.annual_exported_solar_pv_kwh).to round_to_two_digits(2002.43)
      expect(benefits.annual_solar_pv_consumed_onsite_kwh).to round_to_two_digits(11260.15)
