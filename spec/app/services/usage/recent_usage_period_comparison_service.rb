...
describe Usage::RecentUsagePeriodComparisonService, type: :service do
  ...
  context '#recent_usage' do
    it 'creates a resent usage object for results of a baseload period comparison for a given period range' do
      ...
      expect(model.results.kwh).to round_to_two_digits(7817.13) # 7817.125
      expect(model.results.£).to round_to_two_digits(1172.57) # 1172.56875
      expect(model.results.co2).to round_to_two_digits(1524.17) # 1524.167925

      model = service.recent_usage(period_range: -7..-4)
      expect(model.date_range).to eq([Date.new(2022, 5, 8), Date.new(2022, 6, 11)])
      expect(model.results.kwh).to round_to_two_digits(8374.28) # 8374.275
      expect(model.results.£).to round_to_two_digits(1256.14) # 1256.1412500000001
      expect(model.results.co2).to round_to_two_digits(1333.14) # 1333.136225
    end
  end
end