...
class MeterAttributes
  def self.print_attribute_types
  ...
  def self.school_name(mpan_mprn)
    @@school_metadata ||= AnalysticsSchoolAndMeterMetaData.new.mpan_mprn_school_name_map
    @@school_metadata.fetch(mpan_mprn, 'Unknown')
  end

  def self.format_meter_school(mpan_mprn)
    "#{school_name(mpan_mprn)} #{mpan_mprn}"
  end

  def self.primary_groups

    summary  = Hash.new{ |h, k| h[k] = Hash.new(&h.default_proc)}
    METER_ATTRIBUTE_DEFINITIONS.each do |mpan_mprn, attribute_group|
      id = format_meter_school(mpan_mprn)
      attribute_group.each do |type, values|
        if %i[aggregation meter_corrections].include?(type)
          if values.is_a?(Hash)
            values.each do |key, value|
              add(summary, type, key, id)
            end
          else
            values.each do |value|
              if value.is_a?(Hash)
                value.each do |key2, _data|
                  add(summary, type, key2, id)
                end
              else
                add(summary, type, value, id)
              end
            end
          end
        elsif type == :heating_model
          key2 = values.key?(:max_summer_daily_heating_kwh) ? :max_summer_daily_heating_kwh : no_sub_type
          add(summary, type, key2, id)
        elsif type == :function
          values.each do |function|
            add(summary, type, function, id)
          end
        else
          add(summary, type, :no_sub_type, id)
        end
        # summary[type][
      end
    end
    summary.each do |key1, data|
      data.each do |key2, schools|
        puts "#{key1} => #{key2}:"
        schools.each do |school_mpan_mprn|
          puts "    #{school_mpan_mprn}"
        end
      end
    end
  end

  def self.add(summary, key1, key2, mpan_mprn)
    # dig doesnt work here as it creates a 3 level hash
    summary[key1][key2] = Array.new unless summary[key1].key?(key2)
    summary[key1][key2].push(mpan_mprn)
  end
...
MeterAttributes.print_attribute_types
