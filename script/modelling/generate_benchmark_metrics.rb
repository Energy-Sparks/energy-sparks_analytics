# use the average school frame work to generate benchmark and exemplar data
# once average_school_data.rb has been generated and copied into the code base
# run this and copy it as well
#
# pick a random school as a model for holidays etc.
# to calculate meta data for single pupil version of that school
...
def calculate_benchmark_metadata(benchmark_type, fuel_type, school_type, school)
...
example_schools.each do |school_type, name_pattern_match|
  ...
  %i[benchmark exemplar].each do |benchmark_type|
    %i[electricity].each do |fuel_type|
      results[school_type][benchmark_type][fuel_type] = calculate_benchmark_metadata(benchmark_type, fuel_type, school_type, school)
    end
  end
end

dir = TestDirectory.instance.results_directory('AverageSchool')
filename = File.join(dir, 'average_school_benchmarks.rb')

File.open(filename, 'w') do |f|
  f.puts("# auto generated by generate_benchmark_metrics.rb")
  f.puts "class AutoGeneratedBenchmarkMetrics"
  f.puts "  def rawdata"
    f.write results.awesome_inspect
    f.puts
  f.puts "  end"
  f.puts "end"
end
