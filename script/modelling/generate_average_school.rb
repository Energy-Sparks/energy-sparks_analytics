...
class CalculateAverageSchool
  ...
  def save_average_school_data_to_ruby_file(fuel_data, benchmark_type_config, filename = 'Results\average_school_data.rb')
        ...
        type_data.each do |benchmark_type, data|
          ...
          data.each do |school_type, daytype_data|
            ...
            daytype_data.each do |daytype, months_data|
              if daytype == :metadata
                save_metadata_to_ruby(f, months_data)
              else
                f.puts "          #{daytype}: {"
                months_data.sort.each do |month, amr_x48|
                  f.puts "            #{sprintf('%2d',month)} => [ # average 48x intraday kWh/pupil readings for #{Date::MONTHNAMES[month]}"
  ...
  def save_average_school_data_to_csv(fuel_data, filename = 'Results\average_school_data.csv')
    CSV.open(filename, 'w') do |csv|
      fuel_data.each do |fuel_type, type_data|
        type_data.each do |benchmark_type, data|
          data.each do |school_type, daytype_data|
            daytype_data.each do |daytype, months_data|
              next if daytype == :metadata
              months_data.sort.each do |month, amr_x48|
  ...
  def calculate_average_school(type: :benchmark, rank_range: 0.2..0.4, fuel_type: :electricity)
    school_averages = calculate_school_averages(fuel_type)
    ...
    data.each do |fuel_type, benchmark_type_data|
      benchmark_type_data.each do |benchmark_type, school_type_data|
        school_type_data.keys.each do |school_type|
          data[fuel_type][benchmark_type][school_type][:metadata] = calculate_benchmark_metadata(benchmark_type, fuel_type, school_type)
        end
      end
    end
  ...
  def calculate_school_averages(fuel_type)
    @school_type_samples ||= {}
    ...
    @schools.map do |school|
      ...
      # TODO(PH, 12Nov2021) suspect this has started double counting?
      @school_type_samples[school_type] ||= {}
      @school_type_samples[school_type][fuel_type] ||= 0
      @school_type_samples[school_type][fuel_type] += 1
  ...
  # pick a random school as a model for holidays etc.
  # calculate meta data for single pupil version of that school
  ...
  def calculate_benchmark_metadata(type, fuel_type, school_type)
    ...
    school_names = RunTests.resolve_school_list(source_db, [example_schools[school_type]])
    ...
    amr_data = avg.normalised_amr_data(type: type, fuel_type: fuel_type)
...
if true
  ...
  benchmark_type_config = [[:benchmark, 0.2..0.4], [:exemplar, 0.1..0.25]]
