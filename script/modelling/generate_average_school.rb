            ...
            daytype_data.each do |daytype, months_data|
              ...
              months_data.sort.each do |month_or_holiday, amr_x48|
                if month_or_holiday.is_a?(Integer)
                  f.puts "            #{month_or_holiday} => [ # average 48x intraday kWh/pupil readings for #{Date::MONTHNAMES[month_or_holiday]}"
                else
                  f.puts "            #{month_or_holiday}:   [ # average 48x intraday kWh/pupil readings for #{month_or_holiday.to_s.humanize}"
                end
...
if false
  ...
  best_test = ['king-james*','all-sain*','mossbrook*'] # one of each type of school allows test and copying of .rb file
