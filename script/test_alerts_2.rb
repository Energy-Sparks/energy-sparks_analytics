...
script = {
  ...
  # ruby_profiler:            true,
  schools:                  ['Hugh.*'], # ['White.*', 'Trin.*', 'Round.*' ,'St John.*'],
  source:                   :analytics_db, # :aggregated_meter_collection, 
  logger2:                  { name: "./log/reports %{school_name} %{time}.log", format: "%{datetime} %{severity.ljust(5, ' ')}: %{msg}\n" },
  alerts:                   {
    alerts:    nil, # [ AlertOutOfHoursGasUsage ], # nil, 
    control:  {
                # print_alert_banner: true,
                # alerts_history: true,
                print_school_name_banner: true,
                # raw_variables_for_saving
                no_outputs:           %i[front_end_template_data front_end_template_tables front_end_template_table_data], # front_end_template_variables front_end_template_data raw_variables_for_saving],
                save_and_compare:  {
                                      summary:      true,
