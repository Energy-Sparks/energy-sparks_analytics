...
script = {
  ...
  schools:                  ['sir-b*'], # ['*astle*',], # ['White.*', 'Trin.*', 'Round.*' ,'St John.*'],
  schools:                  ['bathamp*'], # ['*astle*',], # ['White.*', 'Trin.*', 'Round.*' ,'St John.*'],
  no_source:                :analytics_db, # :aggregated_meter_collection, 
  source:                   :unvalidated_meter_data,
  logger2:                  { name: "./log/reports %{school_name} %{time}.log", format: "%{datetime} %{severity.ljust(5, ' ')}: %{msg}\n" },
  alerts:                   {
    alerts:   nil,
    alerts: [ AlertEnergyAnnualVersusBenchmark, AlertGasAnnualVersusBenchmark ],
